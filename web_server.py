...
async def startup_warmup():
    """Warm the FPL cache in the background so /api/onboard is fast on first hit."""
    import asyncio, threading
    def _warm():
        try:
            _load()
        except Exception:
            pass  # silently ignore if FPL API is unreachable at boot
...
# ---------------------------------------------------------------------------
# Offline demo snapshot (used when FPL API is unreachable)
# Real data from league 1141763 (Zweden PL) fetched earlier this session.
# ---------------------------------------------------------------------------
_DEMO_MANAGER = {
...
def api_rival_squad(entry_id: int):
    ...
    try:
        ...
        err_str = str(exc)
        is_network_err = any(k in err_str for k in ("NameResolution", "ConnectionPool", "Max retries", "Failed to resolve"))
        if is_network_err:
            # Return empty list — frontend falls back to default RIVAL_SQUAD
            raise HTTPException(503, "FPL API temporarily unreachable")
...
def api_onboard(team_id: int):
    ...
    Falls back to STATE data if the FPL API is temporarily unreachable.
    """
    entry = None
    try:
        ...
        err_str = str(exc)
        is_network_err = any(k in err_str for k in ("NameResolution", "ConnectionPool", "Max retries", "Failed to resolve"))
        # Fall back to demo snapshot for the known demo manager
        if team_id == DEFAULT_MANAGER_ID and is_network_err:
            return {**_DEMO_MANAGER, "_offline": True}
        if is_network_err:
            raise HTTPException(503, "FPL API is temporarily unreachable. Please try again in a moment.")
        raise HTTPException(404, f"Team {team_id} not found. Double-check the ID and try again.")
...
def api_league_rivals(fpl_league_id: int, me: int = DEFAULT_MANAGER_ID):
    ...
    try:
        ...
        if is_network_err and fpl_league_id in _DEMO_RIVALS:
            return _DEMO_RIVALS[fpl_league_id]
        if is_network_err:
            raise HTTPException(503, "FPL API is temporarily unreachable. Please try again in a moment.")
        raise HTTPException(404, f"League {fpl_league_id} not found.")
